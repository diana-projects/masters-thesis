...
class SemanticKittiDataset(Dataset):
    ...
    def load_poses(self):
        ...
        for sequence in self.sequences:
            ...
            calib = self.read_calib(os.path.join(self.data_root, "dataset", "sequences", sequence, "calib.txt"))
    ...
    def load_scans(self):
        ...
        for sequence in self.sequences:
            ...
            glob_path = os.path.join(self.data_root, "dataset", "sequences", sequence, "voxels", "*.bin")
    ...
    def get_data_info(self, index):
        ...
        data_info = dict(img_metas=meta_dict, img=img, target=target)
        return data_info
    ...
    def vox2pix(self, cam_E, cam_k, img_W=None, img_H=None, voxel_size=None, vox_origin=None, scene_size=None):
        ...
        """
        img_W, img_H, voxel_size, vox_origin, scene_size = (
            self.img_W,
            self.img_H,
            self.scale * self.voxel_size,
            self.vox_origin,
            self.scene_size,
        )
        ...
        fov_mask = np.logical_and(
            pix_x >= 0,
            np.logical_and(pix_x < img_W, np.logical_and(pix_y >= 0, np.logical_and(pix_y < img_H, pix_z > 0))),
        )
    ...
    def get_meta_info(self, scan, sequence, frame_id):
        ...
        Returns:
            dict: Meta information that will be passed to the data \
                preprocessing pipelines.
        """
        rgb_path = os.path.join(self.data_root, "dataset", "sequences", sequence, "image_2", frame_id + ".png")
        ...
        # CHANGED HERE
        pts_filename = os.path.join(self.data_root, "dataset", "lidar", sequence, frame_id + ".bin")
        # pts_filename = os.path.join(
        #         self.data_root, "dataset", f"sequences_{self.depthmodel}_lidar/sequences", sequence, "voxels", frame_id + ".pseudo"
        #     )
        ...
        for i in self.target_frames:
            ...
            rgb_path = os.path.join(self.data_root, "dataset", "sequences", sequence, "image_2", target_id + ".png")
            ...
            # )
            pts_filename = os.path.join(self.data_root, "dataset", "lidar", sequence, target_id + ".bin")
            pts = np.fromfile(pts_filename, dtype=np.float32)
            pts = pts.reshape((-1, 4))
